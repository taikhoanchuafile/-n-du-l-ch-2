<?php
   	...
   	$san_sua = mysqli_query($connect,"select * from khachsan where id='".$_GET['id']."'");
?>
...
<?php
	...
	while ($sua = $san_sua->fetch_assoc()){ ?>
		...
		<div>
		  	Địa chỉ:
		    <textarea  rows="4" cols="" title="Viết địa chỉ cho khách sạn" class="tipS" name="diachi" style="width: 100%"><?=$sua['diachi']?></textarea>
		</div>
		 <div>
		 	Giá:
		...
		<div>
		  ...
		  <div>
		  	Nội dung:
		    <textarea  rows="8" cols="" title="Viết nội dung cho khách sạn" class="tipS" name="noidung" style="width: 100%"><?=$sua['noidung']?></textarea>
		   </div>
...
<?php
    ...
    if(isset($_POST["submit"])){
        ...
        $upload = mysqli_query($connect,"UPDATE khachsan SET ten='$ten', diachi='$diachi',gia='$gia', hinh='$hinhanh', noidung='$noidung' WHERE id='".$_GET['id']."'");
        if ($upload) {
           header("location:index.php?com=sua&type=khachsan&id=".$_GET['id']."");
